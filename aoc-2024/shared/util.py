    ...
    if os.path.exists(
        ...
        with open(
            ...
            for example in f.read().split("\n\n-----------------------------\n\n"):
                ...
                refresh = puzzle.answered_a and (answer_b is None or answer_b == "None")
...
def is_descending(sequence: Sequence) -> bool:
    return all(a >= b for a, b in zip(sequence, sequence[1:]))


def get_variants_without_one(sequence: list) -> Generator[list, None, None]:
